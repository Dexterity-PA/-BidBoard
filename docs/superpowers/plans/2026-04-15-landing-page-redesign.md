...
**Goal:** Redesign `app/page.tsx` from a dark Apple-style page to a premium light-mode SaaS landing page matching the spec in `docs/superpowers/specs/2026-04-15-landing-page-redesign.md`.

**Architecture:** Full rewrite of `app/page.tsx` preserving the existing inline component-function pattern. Dark design tokens are replaced with light-mode ones (indigo accent, white/gray-50 surfaces). Five new sections are added (social proof bar, 4-part feature deep-dive, for counselors CTA). Three existing sections are restyled (nav, hero, pricing). Responsive behavior is handled via CSS media-query utility classes in `globals.css` that override inline styles via `!important`. `HeroCard.tsx` and `ScrollReveal.tsx` are untouched.

**Tech Stack:** Next.js 16 App Router (server component), inline React styles + CSS class utilities, DM Sans + Instrument Serif (already wired in layout), `lucide-react` for icons, existing `ScrollReveal` component.
...
|---|---|---|
| `app/globals.css` | Modify | Light-mode hover classes + responsive media-query overrides |
| `app/page.tsx` | Full rewrite | All marketing sections |
| `app/_components/HeroCard.tsx` | No change | Dark scholarship card mockup — keep exactly as-is |
| `app/_components/ScrollReveal.tsx` | No change | Scroll animation wrapper |

---

### Task 1: Add light-mode CSS classes to globals.css

**Files:**
- Modify: `app/globals.css`

- [ ] **Step 1: Append new classes to the bottom of globals.css**

Add the following block after the last existing rule in `app/globals.css`:
...
/* ── Responsive grid overrides (override inline styles) ──────── */
@media (max-width: 768px) {
  ...
  .mkt-hero-h1  { font-size: 42px !important; }
  .mkt-hero-sub { font-size: 17px !important; }
  .mkt-section-h2 { font-size: 32px !important; }
  .mkt-feature-h2 { font-size: 28px !important; }
  .mkt-cta-h2 { font-size: 40px !important; }
}
```
...
```bash
cd /Users/main/PycharmProjects/Bidboard && npm run build 2>&1 | tail -20
```

Expected: no errors. If Tailwind purge flags `.mkt-*` classes as unused, that's fine — they appear in the HTML at runtime.

- [ ] **Step 3: Commit**

```bash
git add app/globals.css
git commit -m "style: add light-mode marketing CSS utility classes"
```

---

### Task 2: Rewrite page.tsx — design tokens, Nav, Hero

**Files:**
- Modify: `app/page.tsx` (replace entire file contents up to and including the `Hero` function)

- [ ] **Step 1: Replace the top of page.tsx (imports through Hero function)**

Replace the file from line 1 through the end of the Hero section. The file will temporarily be incomplete — this is fine, we'll add remaining sections in subsequent tasks. Start with:
...
/* placeholder — remaining sections added in next tasks */
...
```bash
cd /Users/main/PycharmProjects/Bidboard && npm run build 2>&1 | tail -30
```

Expected: `✓ Compiled` with no TypeScript errors. The page renders Nav + Hero only at this point.

- [ ] **Step 3: Commit**

```bash
git add app/page.tsx
git commit -m "feat: landing page redesign — tokens, Nav, Hero (light mode)"
```

---

### Task 3: Add SocialProofBar and HowItWorks sections

**Files:**
- Modify: `app/page.tsx` (insert two new functions before the `LandingPage` export)

- [ ] **Step 1: Add SocialProofBar and HowItWorks functions**

Insert the following two functions into `app/page.tsx` immediately before the `export default function LandingPage()` line (removing the `/* placeholder */` comment):
...
- [ ] **Step 2: Update LandingPage to include the new sections**

Replace the `export default function LandingPage()` body:
...
```bash
git add app/page.tsx
git commit -m "feat: landing page — SocialProofBar + HowItWorks sections"
```

---

### Task 4: Add Feature Deep-Dive (4 alternating sections)

**Files:**
- Modify: `app/page.tsx`

- [ ] **Step 1: Add FeatureText helper + all four mockup components**

Insert the following block before the `export default function LandingPage()` line:
...
- [ ] **Step 2: Add FeatureDeepDive to LandingPage**

Update the `LandingPage` export:
...
```bash
git add app/page.tsx
git commit -m "feat: landing page — FeatureDeepDive with 4 styled-div mockups"
```

---

### Task 5: Add Testimonials and Pricing sections

**Files:**
- Modify: `app/page.tsx`

- [ ] **Step 1: Add Testimonials function**

Insert before the `export default function LandingPage()` line:
...
- [ ] **Step 2: Add Pricing function**

Insert after Testimonials, before the `export default`:
...
- [ ] **Step 3: Add both to LandingPage**
...
```bash
git add app/page.tsx
git commit -m "feat: landing page — Testimonials + Pricing sections"
```

---

### Task 6: Add ForCounselorsCTA, FinalCTA, Footer — wire complete page
...
Insert before `export default function LandingPage()`:
...
- [ ] **Step 2: Wire the complete LandingPage**

Replace the `export default function LandingPage()` with the final version:
...
Expected: `✓ Compiled` with route `ƒ /` listed. No TypeScript errors.

- [ ] **Step 4: Commit**

```bash
git add app/page.tsx
git commit -m "feat: landing page — ForCounselorsCTA, FinalCTA, Footer; complete page wired"
```

---

### Task 7: Final responsive check + nav-link-light footer fix

**Files:**
- Modify: `app/globals.css`

The footer links use `nav-link-light` which turns hover color to `#111827` (dark). On the dark footer that's invisible. Add a separate class.

- [ ] **Step 1: Add footer link hover class to globals.css**

Append to `app/globals.css`:

```css
/* ── Footer link hover (dark bg) ─────────────────────────────── */
.footer-link:hover {
  color: #ffffff !important;
}
```

- [ ] **Step 2: Update Footer links to use the new class**

In `app/page.tsx`, find the footer `{["Privacy", "Terms", "Contact"].map(...)}` block and change `className="nav-link-light"` to `className="footer-link"`:
...
Expected: clean build, `ƒ /` route present.

- [ ] **Step 4: Final commit**

```bash
git add app/page.tsx app/globals.css
git commit -m "fix: footer link hover color for dark background"
```
...
|---|---|---|
| Nav — sticky frosted glass, light mode | Task 2 | ✓ |
| Nav — center links hidden mobile | Task 2 (mkt-hide-mobile) | ✓ |
| Hero — serif headline, subhead, 2 CTAs | Task 2 | ✓ |
| Hero — indigo radial glow | Task 2 | ✓ |
| Hero — HeroCard unchanged | Task 2 (imported as-is) | ✓ |
| Social proof bar — 4 stats | Task 3 | ✓ |
| How it works — 3-step numbered flow | Task 3 | ✓ |
| Feature deep-dive — 4 alternating sections | Task 4 | ✓ |
| Feature deep-dive — styled-div mockups | Task 4 | ✓ |
| Feature deep-dive — reverse mobile order for B+D | Task 4 (mkt-2col-flip) | ✓ |
| Testimonials — 3 cards, name/school/amount | Task 5 | ✓ |
| Pricing — 3 tiers, premium elevated | Task 5 | ✓ |
| For Counselors CTA — text + bullet cards | Task 6 | ✓ |
| Final CTA — dark section, indigo glow | Task 6 | ✓ |
| Footer — logo, tagline, links, copyright | Task 6 | ✓ |
| Footer link hover on dark bg | Task 7 | ✓ |
| Responsive — all grids mobile-first | Tasks 1, 3–6 via CSS classes | ✓ |
| No new dependencies | All tasks | ✓ |
| Auth/dashboard/backend untouched | All tasks | ✓ |
